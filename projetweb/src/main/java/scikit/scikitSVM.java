...
public class scikitSVM extends scikitMethod {

	public scikitSVM(String strAdresse, String strVariable, double propApp, ScriptEngine engine) {
	...
	public double run() {
	    ...
	    try{
	    	...
	    	// on crée  un lecteur de fichier .py pour lire le script du support vector machine ligne à ligne 
	    	
	    	InputStream flux = new FileInputStream("src/coca/svm.py");
	    	InputStreamReader lecture = new InputStreamReader(flux);
	    	BufferedReader buff = new BufferedReader(lecture);
	    	String ligne;
	    	InputStream fluxCF = new FileInputStream("src/coca/svmcm.py");
	    	InputStreamReader lectureCF = new InputStreamReader(fluxCF);
	    	BufferedReader buffCF = new BufferedReader(lectureCF);
	    	String ligneCF;
	    	...
	    	while ((ligne=buff.readLine())!=null){
	    		...
	    		// on évalue le code py associé à la ligne lue
	    	}
	    	
	    	String outputCF="";
	    	StringWriter outputWriterCF=null;
	    	while ((ligneCF=buffCF.readLine())!=null){
	    		outputWriterCF = new StringWriter();
		    	engine.getContext().setWriter(outputWriterCF);
	    		System.out.println(engine.eval(ligneCF));
	    		outputCF = outputWriterCF.toString();
	    		// on évalue le code py associé à la ligne lue
	    	...
	    	String output2CF ="";
	    	if(outputCF.contains(",")) {
	    		output2CF = outputCF.split(",")[0]+"."+outputCF.split(",")[1];
	    	}else {
	    		output2CF = outputCF;
	    	}
	    	...
	    	Integer nbOcc = Integer.parseInt(engine.eval("print(n_val)").toString().substring(2,3));
	    	...
	    	for(int i=1; i <=nbOcc; i++) {
	    		conf[i][0] = engine.eval("print(L_val["+i+"-1])").toString();
	    		conf[0][i] = engine.eval("print(L_val["+i+"+1])").toString();
	    	}
	    	
	    	// les valeurs des effectifs par val pred et réelles
	    	for(int i=1; i <= nbOcc;i++) {
	    		for(int j=1; j <= nbOcc; j++) {
	    			conf[i][j] = engine.eval("print(as.numeric(cf_mat["+i+"]["+j+"]))").toString();
	    		}
	    	}
